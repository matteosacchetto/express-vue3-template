...
const tasks = new Listr(
  [
    ...
    {
      title: `Start the ${info.name}-${info.version} container`,
        ...
        task.newListr((parent) => [
          {
            ...
            task: (ctx, task) =>
              execa('docker', [
                ...
                if (result.stdout.toString() === '') {
                  ctx.isRunning = false;
                  task.title = `Container ${info.name}-${info.version} is not running`;
                } else {
                  ctx.isRunning = true;
                  task.title = `Container ${info.name}-${info.version} is already running`;
                  parent.title = `Container ${info.name}-${info.version} is already running`;
                }
              }),
          },
          {
            ...
            task: (ctx, task) =>
              execa('docker', ['start', `${info.name}-${info.version}`]),
          },
        ]),
    },
  ],
