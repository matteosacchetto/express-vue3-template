...
const tasks = new Listr(
  [
    {
      title: `Check if there is already the image ${info.name}/${info.name}:${info.version}`,
      task: (ctx, task) =>
        execa('docker', [
          'image',
          'inspect',
    ...
    {
      title: `Create image ${info.name}/${info.name}:${info.version}`,
      task: () =>
        execa('docker', [
          'build',
          '-t',
      ...
      retry: 3, // Sometimes it fails => retry
    },
  ],
